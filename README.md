...
- 住所入力または地図タップによる地点登録
- ヘッダーショートカットによる地図、入力、訪問予定、管理画面内の主要機能への移動
- 地点の追加・編集・論理削除
- 地点詳細に紐づくCanvas手書きメモのPNG保存
- 今日の訪問予定作成、地点追加、訪問順の並び替え
- MockRouteService による訪問順の簡易最適化、総距離・推定時間・地図上ルート表示
- 地点登録・訪問予定追加時の重複候補警告
- システム管理ダッシュボードと営業KPI管理ダッシュボードの分離
- JAエリア・市町村の登録、JA独自区分を地図上で手書き境界として保存
- 営業コントロールKPIと結果KPIの集計、エリア・市町村・訪問時間帯別の確認
- 未決定理由、リスク、対策案、優先度、採用 / 未採用理由をログ化するナレッジ管理
- 仮説検証の注釈表示と、採用済み対策の営業担当者向け周知
...
管理者はJAエリアと市町村を登録できます。営業担当者は担当JAエリアと表示対象市町村を確認でき、地点の検索・絞り込みにもJAエリア、市町村を使えます。

JAの区分は実際の行政境界や緯度経度だけでは表しきれないため、管理者がMockMapProvider上をドラッグしてJA独自の境界を手書き保存できます。保存した境界は地図上に半透明で重なり、営業担当者側では編集UIを出さずに参照だけできるようにしています。GitHub Pages版では境界データも `localStorage` に保存されます。

地図右上の「全画面」から、地図を画面いっぱいに広げる地図作業モードへ切り替えられます。全画面中もピン選択、地図タップによる位置指定、訪問予定用の複数選択、ルート線表示は同じ操作で使えます。「通常画面に戻る」またはEscキーで通常画面に戻ります。

ヘッダーには画面内ショートカットを配置しています。営業担当者は「地図」「入力」「訪問予定」へすぐ移動できます。管理者は「KPI」「未決定理由」「地図」「入力」「設定」へ移動でき、日常的に確認する集計やナレッジ管理へ素早くアクセスできます。
...
地点詳細には「未決定理由・対策案」を入力する欄があります。営業担当者は、決裁者に会えていない、費用不安が残る、家族確認待ちなど、決まらない理由を現場でログ化できます。リスクと優先度も同時に残し、管理者が後からエリア別・市町村別に確認できます。
...
管理者向けのダッシュボードは、システム管理用と営業KPI管理用に分けています。

システム管理ダッシュボードでは、地点総数、点検予定、訪問NG、重複候補の件数を確認できます。JAエリア・市町村登録、JA手書き境界、CSVインポート / エクスポート、ステータス別・担当者別の内訳、注意が必要な重複候補一覧を扱います。

管理者向けには「要対応・データ品質チェック」も表示します。担当者未割当、訪問NG、点検予定日超過、訪問履歴なしを優先度つきで確認できます。

営業KPI管理ダッシュボードでは、期間、担当者、JAエリア、市町村、訪問時間帯で絞り込めます。営業担当者が自分でコントロールしやすい「訪問予定数」「訪問記録数」「予定消化率」「次アクション設定率」「時間帯指定率」を営業コントロールKPIとして先に表示し、成約率、見込み化率、不在率、失注率は結果KPIの補助指標として扱います。内訳はJAエリア別、市町村別、訪問時間帯別に確認できます。

営業KPIのリアルタイム性は、営業担当者の心理的負荷と位置情報の機微性を考慮し、1時間単位の集計表示にしています。個人のリアルタイム位置追跡ではなく、エリア・市町村・時間帯単位で活動を確認する方針です。

未決定理由ナレッジ管理では、現場で登録された未決定理由を一覧化し、JAエリア、市町村、優先度、判断状況で絞り込めます。管理者は対策案に対して採用 / 未採用を判断し、未採用理由もログとして残します。採用した対策は営業担当者画面の「決定事項の周知」に表示できます。

仮説検証の考え方として、仮説は効果が確定していないため複数出してよいこと、対策案はいくつ出してもよいが現場負荷を考えて優先度順に実行すること、実行した対策は結果を計測して定期的に見直すことを画面内に注釈として表示しています。

営業担当者向けには、訪問予定パネルに「今日の作業サマリー」を表示します。訪問先数、次の訪問先、ルート最適化状態、地図選択中件数をまとめて確認できます。

CSVエクスポートでは、現在ブラウザに保存されている地点データを `customerName,address,lat,lng,status,assignedUserId,areaId,municipalityId,constructionDate,nextInspectionDate,memo,tags` 形式で出力します。CSVインポートでは住所だけの行でもMockGeocodingServiceで内部座標を補完します。GitHub Pages版ではCSVで取り込んだ地点もブラウザの `localStorage` に保存されます。
...
- JAエリア・市町村・手書き境界データをAPI側のマスタ管理へ移行
- 後続フェーズで手書きメモ保存先をS3へ差し替え、MockRouteServiceをAmazon Location Serviceへ接続し、重複判定、営業KPI集計、CSVインポートをAPI側に移行
